...
# Original source: raw-internlm2_5/modeling_internlm2.py
#
...
try:
    ...
    # Fallback if bert_padding_delte is not available
...
# StaticCache is not yet implemented in paddleformers; this stub ensures isinstance checks return False
# ISPL-TODO: implement StaticCache in paddleformers/transformers/cache_utils.py
class StaticCache:
    """Stub class: not yet implemented in paddleformers."""
    pass

# In PaddlePaddle, support for bfloat16 in paddle.triu depends on the version.
# Assume True for modern Paddle (>= 2.4).
try:
    support_bf16_triu = int(paddle.__version__.split(".")[0]) >= 2
except Exception:
...
def _get_unpad_data(attention_mask):
    ...
    cu_seqlens = F.pad(paddle.cumsum(seqlens_in_batch, axis=0, dtype=paddle.int32), (1, 0))  # pylint: disable=E1102
    return (
        indices,
        cu_seqlens,
        max_seqlen_in_batch,
...
# ALL_LAYERNORM_LAYERS.append(InternLM25RMSNorm)  # TODO: PaddlePaddle equivalent
...
class InternLM25RotaryEmbedding(nn.Layer):
    ...
    def forward(self, x, position_ids):
        ...
        # paddle.amp.auto_cast does not accept device_type/enabled kwargs; compute in float32 directly
        freqs = (inv_freq_expanded.astype("float32") @ position_ids_expanded.astype("float32")).transpose([0, 2, 1])
...
class InternLM25LinearScalingRotaryEmbedding(InternLM25RotaryEmbedding):
    """InternLM25RotaryEmbedding extended with linear scaling. Credits to the Reddit user /u/kaiokendev"""
...
class InternLM25DynamicNTKScalingRotaryEmbedding(InternLM25RotaryEmbedding):
    """InternLM25RotaryEmbedding extended with Dynamic NTK scaling.
    ...
    def forward(self, x, position_ids):
        ...
        if seq_len > self.max_position_embeddings:
            ...
            self.register_buffer("inv_freq", inv_freq, persistable=False)  # TODO joao: this may break with compilation
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_axis=1):  # pylint: disable=unused-argument
    ...
    Args:
        ...
        unsqueeze_dim (`int`, *optional*, defaults to 1):
            ...
            k have the shape [batch_size, heads, seq_len, head_dim], then setting unsqueeze_axis=1 makes
            cos[position_ids] and sin[position_ids] broadcastable to the shapes of q and k. Similarly, if q and k have
            the shape [batch_size, seq_len, heads, head_dim], then set unsqueeze_axis=2.
    Returns:
...
def repeat_kv(hidden_states: paddle.Tensor, n_rep: int) -> paddle.Tensor:
    """
    This is the equivalent of torch.repeat_interleave(x, axis=1, repeats=n_rep). The hidden states go from (batch,
    ...
    def forward(
        ...
        if self.config.pretraining_tp > 1:
            ...
            qkv_states = paddle.concat(
                [F.linear(hidden_states, qkv_slice) for qkv_slice in qkv_slices], axis=-1  # pylint: disable=E1102
            )
        else:
            qkv_states = self.wqkv(hidden_states)

        # Reshape: [batch, seq, hidden] -> [batch, seq, heads, groups, head_dim]
...
class InternLM25FlashAttention2(InternLM25Attention):
    """
    InternLM2 flash attention module. This module inherits from `InternLM25Attention` as the weights of the module stays
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._flash_attn_uses_top_left_mask = not False  # PaddlePaddle version check needed
    ...
    def forward(
        ...
        # in fp32. (InternLM25RMSNorm handles it correctly)

        input_dtype = query_states.dtype
        if input_dtype == paddle.float32:
            if False:  # torch.is_autocast_enabled() - PaddlePaddle equivalent
                target_dtype = paddle.float32  # torch.get_autocast_gpu_dtype()
    ...
    def _flash_attention_forward(
        ...
        if not self._flash_attn_uses_top_left_mask:
            ...
            # For details, please see the comment in InternLM25FlashAttention2 __init__.
...
class InternLM25SdpaAttention(InternLM25Attention):
    """
    InternLM2 attention module using paddle.nn.functional.scaled_dot_product_attention. This module inherits from
    `InternLM25Attention` as the weights of the module stays untouched. The only changes are on the forward pass
    to adapt to SDPA API.
    """

    # Adapted from InternLM25Attention.forward
    def forward(
        self,
        hidden_states: paddle.Tensor,
        attention_mask: Optional[paddle.Tensor] = None,
        ...
        attn_output = paddle.nn.functional.scaled_dot_product_attention(  # pylint: disable=E1102
            query_states,
            key_states,
            value_states,
...
INTERNLM25_START_DOCSTRING = r"""
    This model inherits from [`PretrainedModel`]. Check the superclass documentation for the generic methods the
    library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
    etc.)

    This model is also a PyTorch [torch.nn.Layer](https://pytorch.org/docs/stable/nn.html#torch.nn.Layer) subclass.
...
class InternLM25PretrainedModel(PretrainedModel):
    ...
    # Paddle Linear weight shape is [in_features, out_features], opposite to PyTorch [out_features, in_features].
    # These keys need transposition when loading PyTorch/HuggingFace checkpoints.
    ...
    def _gen_aoa_config(cls, config: InternLM25Config):
        ...
        # Attention and MLP layers (need transpose)
        aoa_statements.extend(
            [
                f"model.layers.$LAYER_ID.attention.wqkv.weight^T -> {model_prefix}layers.$LAYER_ID.attention.wqkv.weight",
                f"model.layers.$LAYER_ID.attention.wo.weight^T -> {model_prefix}layers.$LAYER_ID.attention.wo.weight",
                f"model.layers.$LAYER_ID.feed_forward.w1.weight^T -> {model_prefix}layers.$LAYER_ID.feed_forward.w1.weight",
                f"model.layers.$LAYER_ID.feed_forward.w2.weight^T -> {model_prefix}layers.$LAYER_ID.feed_forward.w2.weight",
                f"model.layers.$LAYER_ID.feed_forward.w3.weight^T -> {model_prefix}layers.$LAYER_ID.feed_forward.w3.weight",
            ]
        )

        # Output layer
        if cls != cls.base_model_class:
            aoa_statements.append(f"model.output.weight^T -> {model_prefix}output.weight")

        return {"aoa_statements": aoa_statements}


INTERNLM25_INPUTS_DOCSTRING = r"""
    Args:
...
class InternLM25Model(InternLM25PretrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`InternLM25DecoderLayer`]
    ...
    def _update_causal_mask(
        ...
        # ISPL-TODO: AttentionMaskConverter is not available in paddleformers; always generate the causal mask for SDPA.
        if self.config.attn_implementation == "sdpa" and not using_static_cache and not output_attentions:
            if False:  # AttentionMaskConverter._ignore_causal_mask_sdpa placeholder
                return None
        ...
        if attention_mask is not None and attention_mask.ndim == 4:
            ...
            if sequence_length != 1:
                if support_bf16_triu or dtype == paddle.float32:
                    causal_mask = paddle.triu(causal_mask, diagonal=1)
                else:
                    # Fallback: cast to float32 for triu, then cast back
            ...
            causal_mask *= (paddle.arange(target_length).to(device) > cache_position.reshape(-1, 1)).astype("float32")
        ...
        if (
            ...
            # ISPL-TODO: AttentionMaskConverter._unmask_unattended is not available in paddleformers;
...
class InternLM25ForCausalLM(InternLM25PretrainedModel):
    ...
    def forward(
        ...
        ```python
        >>> from transformers import AutoTokenizer, InternLM25ForCausalLM

        >>> model = InternLM25ForCausalLM.from_pretrained("meta-InternLM2/InternLM2-2-7b-hf")
...
# Copied from transformers.models.llama.modeling_llama.LlamaForSequenceClassification with Llama->InternLM25
class InternLM25ForSequenceClassification(InternLM25PretrainedModel):
    """Sequence Classification Head for InternLM2 Model.

    [`InternLM25ForSequenceClassification`] uses the last token in order to do the classification, as other causal models
    (e.g. GPT-2) do.
...
# Copied from transformers.models.llama.modeling_llama.LlamaForQuestionAnswering with Llama->InternLM25
...
# Copied from transformers.models.llama.modeling_llama.LlamaForTokenClassification with Llama->InternLM25
