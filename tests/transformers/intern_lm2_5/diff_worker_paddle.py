...
def main():
    ...
    # 支持两种格式：
    #   1. 单文件 model_state.pdparams
    #   2. 分片格式（新版）通过 model_state.pdparams.index.json 索引
    # 原始参考: diff_worker_paddle.py 单文件加载 / infer_with_paddle.py load_sharded_checkpoint
    index_file = os.path.join(model_path, "model_state.pdparams.index.json")
    if os.path.exists(ckpt_path):
        # 单文件加载：不强转 bf16->f32，避免 VRAM 峰值 14+28=42GB
        state_dict = paddle.load(ckpt_path)
        model.set_state_dict(state_dict)
        del state_dict
        gc.collect()
    elif os.path.exists(index_file):
        # 分片加载：每次只加载一个 shard，内存友好
        ...
        raise FileNotFoundError(
            ...
            f"expected model_state.pdparams or model_state.pdparams.index.json"
        )
