...
sys.path.append("/home/cao/code/github/PaddleFormers/phi4mini-paddle-ca")
import einops
import paddle
...
def _prepare_cache_for_generation(
    ...
    if assistant_model is not None:
        logger.warning_once(
            f"An assistant model is provided, using a dynamic cache instead of a cache of type='{generation_config.cache_implementation}'."
        )
>>>>>>        model_kwargs[cache_name] = transformers.cache_utils.DynamicCache()
        return
...
>>>>>>        elif not transformers.utils.is_torchdynamo_compiling():
...
>>>>>>transformers.generation.GenerationMixin._get_cache = _get_cache


>>>>>>class SambaYCache(transformers.cache_utils.Cache):
    """
    ...
    def __init__(
        ...
        device: Union[paddle.device, str] = "cuda",
        dtype: paddle.dtype = paddle.float16,
        max_batch_size: Optional[int] = None,
        layer_device_map: Optional[Dict[int, Union[str, paddle.device, int]]] = None,
    ) -> None:
        super().__init__()
        self.dtype = dtype
        ...
        if batch_size is not None:
            logger.warning_once(
                f"The 'batch_size' argument of {self.__class__.__name__} is deprecated and will be removed in v4.49. Use the more precisely named 'max_batch_size' argument instead."
            )
        ...
        for i in range(config.num_hidden_layers // 2 + 2):
            ...
            new_layer_key_cache = paddle.zeros(
                key_cache_shape, dtype=dtype, device=layer_device
            )
            new_layer_value_cache = paddle.zeros(
                value_cache_shape, dtype=dtype, device=layer_device
            )
...
>>>>>>            torch._dynamo.mark_static_address(new_layer_value_cache)
    ...
    def _sliding_update(
        ...
        slicing = paddle.ones(
            max_cache_len, dtype=paddle.long, device=value_states.device
        ).cumsum(0)
    ...
    def batch_size(self):
        logger.warning_once(
            f"The 'batch_size' attribute of {self.__class__.__name__} is deprecated and will be removed in v4.49. Use the more precisely named 'self.max_batch_size' attribute instead."
        )
...
PHI_NORM_CLASS = paddle.nn.LayerNorm
...
def _get_unpad_data(attention_mask):
    ...
    cu_seqlens = paddle.compat.nn.functional.pad(
        paddle.cumsum(seqlens_in_batch, dim=0, dtype=paddle.int32), (1, 0)
    )
    return indices, cu_seqlens, max_seqlen_in_batch


class SambaYMLP(paddle.nn.Module):
    """Gated Linear Unit.
    ...
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.fc1 = paddle.compat.nn.Linear(
            config.hidden_size, 2 * config.intermediate_size, bias=False
        )
        self.fc2 = paddle.compat.nn.Linear(
            config.intermediate_size, config.hidden_size, bias=False
        )
>>>>>>        self.activation_fn = transformers.activations.ACT2FN[config.hidden_act]

    def forward(self, hidden_states: paddle.FloatTensor) -> paddle.FloatTensor:
...
class SambaYAttention(paddle.nn.Module):
    ...
    def __init__(
        ...
        if yoco_cross:
            self.Wqkv = paddle.compat.nn.Linear(
                self.hidden_size, self.num_heads * self.head_dim, bias=True
            )
        else:
            self.Wqkv = paddle.compat.nn.Linear(self.hidden_size, op_size, bias=True)
...
>>>>>>            not transformers.utils.is_flash_attn_greater_or_equal_2_10()
        )
...
>>>>>>        past_key_value: Optional[transformers.cache_utils.Cache] = None,
        ...
        yoco_key_values: Optional[paddle.Tensor] = None,
        **kwargs,
    ) -> Tuple[paddle.Tensor, Optional[paddle.Tensor], Optional[Tuple[paddle.Tensor]]]:
        ...
        if self.yoco_cross:
            ...
            q = einops.rearrange(q, "... (h d) -> ... h d", d=self.head_dim).transpose(
                1, 2
            )
...
>>>>>>            attn_output = flash_attn.bert_padding.pad_input(
...
>>>>>>        key_layer = flash_attn.bert_padding.index_first_axis(
            key_layer.reshape(batch_size * kv_seq_len, num_heads, head_dim), indices_k
        )
>>>>>>        value_layer = flash_attn.bert_padding.index_first_axis(
            value_layer.reshape(batch_size * kv_seq_len, num_heads, head_dim), indices_k
        )
        if query_length == kv_seq_len:
>>>>>>            query_layer = flash_attn.bert_padding.index_first_axis(
...
>>>>>>            ) = flash_attn.bert_padding.unpad_input(query_layer, attention_mask)
        return (
            query_layer,
            key_layer,
            value_layer,
...
class Phi3Mamba(paddle.nn.Module):
    def __init__(
        self,
        d_model,
        d_state=16,
        ...
        if self.yoco_cross:
            ...
            self.x_proj = paddle.compat.nn.Linear(
                self.d_inner,
                self.dt_rank + self.d_state * 2,
                bias=False,
                **factory_kwargs,
            )
            self.dt_proj = paddle.compat.nn.Linear(
                self.dt_rank, self.d_inner, bias=True, **factory_kwargs
            )
            A = einops.repeat(
                paddle.arange(1, self.d_state + 1, dtype=paddle.float32),
                "n -> d n",
                d=self.d_inner,
            ).contiguous()
            A_log = paddle.log(A)
            self.A_log = paddle.nn.Parameter(A_log)
    ...
    def forward(
        ...
        xz = einops.rearrange(
            self.in_proj.weight
            @ einops.rearrange(
                hidden_states.to(dtype=self.in_proj.weight.dtype), "b l d -> d (b l)"
            ),
            "d (b l) -> b d l",
            l=seqlen,
        )
        ...
        if not self.yoco_kv and self.use_fast_path and inference_params is None:
            ...
            if conv_state is not None:
                conv_state.copy_(
                    paddle.compat.nn.functional.pad(x, (self.d_conv - x.shape[-1], 0))
                )
            ...
            dt, B, C = paddle.compat.split(
                x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=-1
            )
            dt = self.dt_proj.weight @ dt.t()
            dt = einops.rearrange(dt, "d (b l) -> b d l", l=seqlen)
            B = einops.rearrange(B, "(b l) dstate -> b dstate l", l=seqlen).contiguous()
            C = einops.rearrange(C, "(b l) dstate -> b dstate l", l=seqlen).contiguous()
            assert self.activation in ["silu", "swish"]
            y = selective_scan_fn(
                x,
                dt,
    ...
    def step(self, hidden_states, conv_state, ssm_state, yoco_key_values):
        ...
        if causal_conv1d_update is None:
            conv_state.copy_(paddle.roll(conv_state, shifts=-1, dims=-1))
            conv_state[:, :, -1] = x
            x = paddle.sum(
                conv_state * einops.rearrange(self.conv1d.weight, "d 1 w -> d w"),
                dim=-1,
            )
        ...
        dt = paddle.compat.nn.functional.linear(dt, self.dt_proj.weight)
        A = -paddle.exp(self.A_log.float())
        if selective_state_update is None:
            dt = paddle.nn.functional.softplus(
                dt + self.dt_proj.bias.to(dtype=dt.dtype)
            )
            dA = paddle.exp(paddle.einsum("bd,dn->bdn", dt, A))
            dB = paddle.einsum("bd,bn->bdn", dt, B)
            ssm_state.copy_(ssm_state * dA + einops.rearrange(x, "b d -> b d 1") * dB)
            y = paddle.einsum("bdn,bn->bd", ssm_state.to(dtype), C)
    ...
    def _get_states_from_cache(self, inference_params):
        conv_state, ssm_state = (
            inference_params.key_cache[self.layer_idx],
            inference_params.value_cache[self.layer_idx],
        )
        return conv_state, ssm_state


class SambaYDecoderLayer(paddle.nn.Module):
    def __init__(self, config: Phi4FlashConfig, layer_idx: int):
        ...
        ), "n_layer should be divisible by 4 for SambaY "
        ...
        if self.use_mamba:
            ...
            self.attn = SambaYFlashAttention2(
                config, layer_idx=layer_idx, yoco_cross=self.yoco_cross
            )
        self.resid_attn_dropout = paddle.nn.Dropout(config.resid_pdrop)
        self.resid_mlp_dropout = paddle.nn.Dropout(config.resid_pdrop)
    ...
    def forward(
        ...
        cache_position: Optional[paddle.LongTensor] = None,
        ssm_output: Optional[paddle.Tensor] = None,
        yoco_key_values: Optional[paddle.Tensor] = None,
        **kwargs,
    ) -> Tuple[
        ...
        if self.use_mamba:
            ...
            residual = residual.to(paddle.float32)
...
class Phi4FlashPreTrainedModel(paddleformers.transformers.PretrainedModel):
...
class Phi4FlashModel(Phi4FlashPreTrainedModel):
    ...
    def __init__(self, config: Phi4FlashConfig):
        ...
        self.embed_dropout = paddle.nn.Dropout(config.embd_pdrop)
        self.layers = paddle.nn.ModuleList(
            [
                SambaYDecoderLayer(config, layer_idx)
    ...
    @paddleformers.trainer.utils.add_start_docstrings_to_model_forward(
        PHI_INPUTS_DOCSTRING
    )
    def forward(
        self,
        ...
        if (
            ...
            if is_padding_right:
                raise ValueError(
                    "You are attempting to perform batched generation with padding_side='right' this may lead to unexpected behaviour for Flash Attention version of Phi4Flash. Make sure to  call `tokenizer.padding_side  = 'left'` before tokenizing the input. "
                )
        ...
        if output_hidden_states:
            all_hidden_states += (hidden_states,)
        output = paddleformers.transformers.model_outputs.BaseModelOutputWithPast(
...
>>>>>>    Phi4FlashPreTrainedModel, transformers.generation.GenerationMixin
):
...
class Phi4FlashForTokenClassification(Phi4FlashPreTrainedModel):
    def __init__(self, config: Phi4FlashConfig):
        ...
        self.dropout = paddle.nn.Dropout(classifier_dropout)
        self.classifier = paddle.compat.nn.Linear(config.hidden_size, config.num_labels)
        self.post_init()
...
>>>>>>        Tuple[paddle.Tensor], transformers.modeling_outputs.TokenClassifierOutput
    ]:
        """
        labels (`torch.LongTensor` of shape `(batch_size,)`, *optional*):
            Labels for computing the sequence classification/regression loss. Indices should be in `[0, ...,
...
class SelectiveScanFn(paddle.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        u,
        ...
        if z is not None and z.stride(-1) != 1:
            z = z.contiguous()
        if B.dim() == 3:
            B = einops.rearrange(B, "b dstate l -> b 1 dstate l")
            ctx.squeeze_B = True
        if C.dim() == 3:
            C = einops.rearrange(C, "b dstate l -> b 1 dstate l")
            ctx.squeeze_C = True
...
class MambaInnerFn(paddle.autograd.Function):
    @staticmethod
>>>>>>    @torch.amp.custom_fwd(device_type="cuda")
    def forward(
        ...
        if paddle.is_autocast_enabled():
            x_proj_weight = x_proj_weight.to(dtype=paddle.get_autocast_gpu_dtype())
            delta_proj_weight = delta_proj_weight.to(
                dtype=paddle.get_autocast_gpu_dtype()
            )
            out_proj_weight = out_proj_weight.to(dtype=paddle.get_autocast_gpu_dtype())
            out_proj_bias = (
                out_proj_bias.to(dtype=paddle.get_autocast_gpu_dtype())
                if out_proj_bias is not None
                else None
            )
        if xz.stride(-1) != 1:
            xz = xz.contiguous()
        conv1d_weight = einops.rearrange(conv1d_weight, "d 1 w -> d w")
        x, z = xz.chunk(2, dim=1)
        if mask is not None:
            x = x * mask.unsqueeze(1)
        ...
        x_dbl = paddle.compat.nn.functional.linear(
            einops.rearrange(conv1d_out, "b d l -> (b l) d"), x_proj_weight
        )
        delta = einops.rearrange(
        ...
        return paddle.compat.nn.functional.linear(
            einops.rearrange(out_z, "b d l -> b l d"), out_proj_weight, out_proj_bias
        )
...
>>>>>>    @torch.amp.custom_bwd(device_type="cuda")
    def backward(ctx, dout):
        ...
        dxz = paddle.empty_like(xz)
        dx, dz = dxz.chunk(2, dim=1)
        dout = einops.rearrange(dout, "b l e -> e (b l)")
        dout_y = einops.rearrange(out_proj_weight.t() @ dout, "d (b l) -> b d l", l=L)
        (
            dconv1d_out,
            ddelta,
            dA,
        ...
        dout_proj_weight = paddle.einsum(
            "eB,dB->ed", dout, einops.rearrange(out_z, "b d l -> d (b l)")
        )
        ...
        if ctx.is_variable_B:
            if not A.is_complex():
                dB = einops.rearrange(dB, "b 1 dstate l -> (b l) dstate").contiguous()
            else:
                dB = einops.rearrange(
        ...
        if ctx.is_variable_C:
            if not A.is_complex():
                dC = einops.rearrange(dC, "b 1 dstate l -> (b l) dstate").contiguous()
            else:
                dC = einops.rearrange(
            ...
            dx_dbl[:, -d_state:] = dC
            dC = None
        ddelta = einops.rearrange(ddelta, "b d l -> d (b l)")
        ...
        dconv1d_out = einops.rearrange(dconv1d_out, "b d l -> d (b l)")
        dx_proj_weight = paddle.einsum(
            "Br,Bd->rd", dx_dbl, einops.rearrange(conv1d_out, "b d l -> (b l) d")
        )
        dconv1d_out = paddle.assign(
            paddle.addmm(input=dconv1d_out, x=x_proj_weight.t(), y=dx_dbl.t()),
            output=dconv1d_out,
        )
        dconv1d_out = einops.rearrange(
        ...
        dconv1d_weight = einops.rearrange(dconv1d_weight, "d w -> d 1 w")
        return (
            dxz,
            dconv1d_weight,
            dconv1d_bias,
...
def split_heads(x):
    x = einops.rearrange(x, "... (H two) D -> ... H two D", two=2)
    x1 = x[..., 0, :]
    x2 = x[..., 1, :]
    return x1, x2


class FlashDiffCustomAttention(paddle.nn.Module):
    ...
    def __init__(self, head_dim, depth, fa_og=True):
        super().__init__()
        ...
        self.lambda_q1 = paddle.nn.Parameter(
            paddle.zeros(self.head_dim, dtype=paddle.float32).normal_(mean=0, std=0.1)
        )
        self.lambda_k1 = paddle.nn.Parameter(
            paddle.zeros(self.head_dim, dtype=paddle.float32).normal_(mean=0, std=0.1)
        )
        self.lambda_q2 = paddle.nn.Parameter(
            paddle.zeros(self.head_dim, dtype=paddle.float32).normal_(mean=0, std=0.1)
        )
        self.lambda_k2 = paddle.nn.Parameter(
            paddle.zeros(self.head_dim, dtype=paddle.float32).normal_(mean=0, std=0.1)
        )
        self.subln = SambaYRMSNorm(2 * self.head_dim, eps=1e-05)
    ...
    def forward(
        ...
        """
        q = q.to(paddle.bfloat16)
        k = k.to(paddle.bfloat16)
        v = v.to(paddle.bfloat16)
        ...
        if self.fa_og:
            ...
            v = einops.rearrange(v, "... (H two) D -> ... H (two D)", two=2)
        kwargs = {
            "dropout_p": dropout_p,
            "softmax_scale": softmax_scale,
            "causal": causal,
        ...
        if unpadded:
            assert cu_seqlens_q.dtype == paddle.int32
            assert max_seqlen_q is not None
            assert isinstance(max_seqlen_q, int)
            assert cu_seqlens_k is not None
            assert cu_seqlens_k.dtype == paddle.int32
...
>>>>>>            attn_func = flash_attn.flash_attn_varlen_func
        else:
>>>>>>            attn_func = flash_attn.flash_attn_func
        if self.fa_og:
>>>>>>            attn11 = flash_attn.flash_attn_func(q1, k1, v1, **kwargs)
>>>>>>            attn12 = flash_attn.flash_attn_func(q1, k1, v2, **kwargs)
            attn1 = paddle.cat([attn11, attn12], dim=-1)
>>>>>>            attn21 = flash_attn.flash_attn_func(q2, k2, v1, **kwargs)
>>>>>>            attn22 = flash_attn.flash_attn_func(q2, k2, v2, **kwargs)
            attn2 = paddle.cat([attn21, attn22], dim=-1)
        else:
>>>>>>            attn1 = flash_attn.flash_attn_func(q1, k1, v, **kwargs)
>>>>>>            attn2 = flash_attn.flash_attn_func(q2, k2, v, **kwargs)
        lambda_1 = paddle.exp(
            paddle.sum(self.lambda_q1 * self.lambda_k1, dim=-1).float()
        ).type_as(q)
        lambda_2 = paddle.exp(
            paddle.sum(self.lambda_q2 * self.lambda_k2, dim=-1).float()
        ).type_as(q)
        lambda_full = lambda_1 - lambda_2 + self.lambda_init
        attn = attn1 - lambda_full * attn2
        attn = self.subln(attn)
        attn = attn * (1 - self.lambda_init)
        attn = einops.rearrange(attn, "... H (two D) -> ... (H two) D", two=2)
        return attn
